...
public class WriteController2 extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		req.getRequestDispatcher("/admin/Write2.jsp").forward(req, resp);
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if( mr==null) {
			...
			JSFunction.alertLocation(resp, "첨부 파일이 제한 용량을 초과합니다.", "../myboard/write2.do");
			return;
		}
		...
		//한번에 여러 게시물 입력하기 (왜 같은 코드인데 WriteController notice 쪽만 작동하지?)
		/*int iResult = 0;
		for (int i=1; i<=100; i++) {
		...
		if(result == 1) {
			...
			resp.sendRedirect("../myboard/list2.do");
		}
		else {
			//insert에 실패하면 작성 페이지로 다시 이동한다.
			resp.sendRedirect("../myboard/write2.do");
		}
