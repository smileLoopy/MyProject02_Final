...
public class MyBoardDAO extends DBConnPool{
	...
	//게시판 목록 출력
	public List<MyBoardDTO> selectlist(Map<String, Object> map){
		
		
		List<MyBoardDTO> board = new ArrayList<MyBoardDTO>();
	...
	//글쓰기
	public int insertWrite(MyBoardDTO dto) {
		
		int result = 0;
		
		try {
			
			String query = "INSERT INTO myboard (name, title, content, ofile, sfile, pass) "
					+ " VALUES (?, ?, ?, ?, ?, ?) ";
			psmt = con.prepareStatement(query);
	...
	public void downCountPluse(String idx) {
		String query = "UPDATE myboard SET downcount=downcount+1 WHERE idx=?";
		try {
			psmt = con.prepareStatement(query);
	...
	//상세보기
	public MyBoardDTO selectView (String idx) {
		
		//조회된 레코드를 DTO객체에 저장한 후 반환한다.
		MyBoardDTO dto = new MyBoardDTO();
		//인파라미터가 있는 쿼리문 작성
		String query = "SELECT * FROM myboard WHERE idx=?";
	...
	public void updateViewCount(String idx) {
		String query = "UPDATE myboard SET viewcount=viewcount+1 WHERE idx=?";
		try {
			psmt = con.prepareStatement(query);
			psmt.setString(1, idx);
			psmt.executeQuery();
	...
	public boolean confirmPassword(String pass, String idx) {
		boolean isCorr = true;
		try {
			String sql = "SELECT COUNT(*) FROM myboard WHERE pass=? AND idx=?";
	...
	public int deletePost (String idx) {
		int result = 0;
		try {
			String query = "DELETE FROM myboard WHERE idx=?";
			psmt = con.prepareStatement(query);
			psmt.setString(1, idx);
			result = psmt.executeUpdate();
		}
	...
	//게시물 수정 처리 
	public int updatePost (MyBoardDTO dto) {
		int result = 0;
		try {
			//update쿼리문 작성
			String query = "UPDATE myboard SET title=?, name=?, content=?, ofile=?, sfile=? WHERE idx=? and pass=?";
