...
@WebServlet("/mynotice/edit_notice.do")
public class EditController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		MyNoticeDTO dto = dao.selectView(idx);
		req.setAttribute("dto", dto);
		req.getRequestDispatcher("/admin/Edit_notice.jsp").forward(req, resp);
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if( mr==null) {
			...
			JSFunction.alertLocation(resp, "제한용량초과 or 디렉토리 경로를 확안하세요.", "../mynotice/write_notice.do");
			return;
		}
		
		//파알을 제외한 나머지 폼값을 얻어온다.
		...
		if(fileName != null) {
			...
			//새로운 파일을 등록하지 않는 경우 파일명을 DTO에 저장한다.
		...
		if(result == 1) {
			//수정이 완료되면 session에 저장된 패스워드를 지운 후 상서보기 화면으로 이동한다.
			session.removeAttribute("pass");
			resp.sendRedirect("../mynotice/view_notice.do?idx=" + idx);
		}
		else {
			JSFunction.alertLocation(resp, "비밀번호 검증을 다시 진행해주세요", "../mynotice/view_notice.do?idx=" + idx);
		}
