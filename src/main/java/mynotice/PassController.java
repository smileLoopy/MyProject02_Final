...
//어노테이션을 통한 요청명 매핑
@WebServlet("/mynotice/pass_notice.do")
public class PassController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		req.getRequestDispatcher("/admin/Pass_notice.jsp").forward(req, resp);
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(confirmed) {
			...
			if(mode.equals("edit")) {
				...
				resp.sendRedirect("../mynotice/edit_notice.do?idx="+ idx);
			}
				...
				//커넥션풀에 자원 반납
				dao.close();
				//게시물 삭제에 성공한 경우에만 첨부된 파일을 삭제한다 (기존 자료 얻어오는 이유는 첨부파일때문)
				if(result==1) {
				...
				JSFunction.alertLocation(resp, "삭제되었습니다.", "../mynotice/list_notice.do");
