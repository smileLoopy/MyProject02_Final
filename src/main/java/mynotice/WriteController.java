 ...
 */
@WebServlet("/mynotice/write_notice.do")
public class WriteController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		req.getRequestDispatcher("/admin/Write_notice.jsp").forward(req, resp);
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if( mr==null) {
			...
			JSFunction.alertLocation(resp, "첨부 파일이 제한 용량을 초과합니다.", "../notice/write_notice.do");
			return;
		}
		...
		if(result == 1) {
			...
			resp.sendRedirect("../mynotice/list_notice.do");
		}
		else {
			//insert에 실패하면 작성 페이지로 다시 이동한다.
			resp.sendRedirect("../mynotice/write_notice.do");
		}
